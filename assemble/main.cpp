...
#pragma pack(pop)
    ...
    private:
        bool apply_modrmsib(argtype_t type, argument_t arg, modrm_t& target_modrm, sib_t& target_sib, uint32_t* sibdisp)
        {
    ...
    public:
        ...
        error emit(buffervec<uint8_t>& assembly) noexcept
        {
            ...
            {
                for (uint8_t i = 0; i < opcode._f_opcode_count; ++i)
                {
                    if (!assembly.push(opcode._f_opcode_extra[i])) return __error(errors::out_of_memory);
                }
            }
            ...
            if ((!opcode.flags.has(opcode_flags_t::register_adjusted) && signature.arg1.is_modrm()) || signature.arg2.is_modrm())
            {
                ...
                uint32_t sib_disp = 0;
                bool needs_sib = apply_modrmsib(signature.arg1, arg1, modrm, sib, &sib_disp) 
                                 || apply_modrmsib(signature.arg2, arg2, modrm, sib, &sib_disp);

                if (opcode.flags.has(opcode_flags_t::reg_extended))
                {
                    modrm.reg = opcode._f_regextender;
                }
    ...
    umap<signature_t, opcode_t, value_type_hash<signature_t>> opcode_map {
        ...
        { { "and",  argtype_t::EAX,      argtype_t::imm32    }, { 0x25, s("and the contents of EAX with an immediate 32-bit value and store the result in EAX.") } },
        { { "and",  argtype_t::RAX,      argtype_t::imm32    }, { 0x25, s("and the contents of RAX with an immediate 32-bit value and store the result in RAX.") } },
        { { "and",  argtype_t::regmem32, argtype_t::imm32    }, { 0x81, s("and the contents of reg/mem32 with imm32."),                 opcode_flags_t::reg_extended, 4 } },
        { { "and",  argtype_t::regmem64, argtype_t::imm32    }, { 0x81, s("and the contents of reg/mem64 with a sign-extended imm32."), opcode_flags_t::reg_extended, 4 } },
        { { "and",  argtype_t::regmem32, argtype_t::imm8     }, { 0x83, s("and the contents of reg/mem32 with a sign-extended imm8"),   opcode_flags_t::reg_extended, 4 } },
        { { "and",  argtype_t::regmem64, argtype_t::imm8     }, { 0x83, s("and the contents of reg/mem64 with a sign-extended imm8"),   opcode_flags_t::reg_extended, 4 } },
        { { "and",  argtype_t::regmem32, argtype_t::reg32    }, { 0x21, s("and the contents of a 32 bit register or memory location with the contents of a 32-bit register") } },
        { { "and",  argtype_t::regmem64, argtype_t::reg64    }, { 0x21, s("and the contents of a 64-bit register or memory location with the contents of a 64-bit register") } },
        { { "and",  argtype_t::reg32,    argtype_t::regmem32 }, { 0x23, s("and the contents of a 32-bit register with the contents of a 32-bit memory location or register.") } },
        { { "and",  argtype_t::reg64,    argtype_t::regmem64 }, { 0x23, s("and the contents of a 64-bit register with the contents of a 64-bit memory location or register.") } },



        { { "cpuid",  argtype_t::unused, argtype_t::unused   }, { 0x0F, 
                                                                  s("Returns information about the processor and its capabilities. EAX specifies the function number, and the data is returned in EAX, EBX, ECX, EDX."), 
        ...
        { { "mov",  argtype_t::regmem64, argtype_t::imm32    }, { 0xC7, s("Move a 32-bit immediate value into a 64-bit register or memory operand") } },


        { { "pop",  argtype_t::regmem64, argtype_t::unused   }, { 0x8F, s("Pop the top of the stack into a 64-bit register or memory.") } },
        { { "pop",  argtype_t::reg64,    argtype_t::unused   }, { 0x58, s("Pop the top of the stack into a 64-bit register."), opcode_flags_t::register_adjusted } },
        ...
        { { "push", argtype_t::regmem64, argtype_t::unused   }, { 0xFF, s("Push the contents of a 64-bit register or memory operand onto the stack."), opcode_flags_t::reg_extended, 6 } },
        { { "push", argtype_t::reg64,    argtype_t::unused   }, { 0x50, s("Push the contents of a 64-bit register onto the stack."), opcode_flags_t::register_adjusted } },
        { { "push", argtype_t::imm32,    argtype_t::unused   }, { 0x68, s("Push a sign-extended 32-bit immediate value onto the stack.") } },

        { { "ret",  argtype_t::unused,   argtype_t::unused   }, { 0xC3, s("Near return to the calling procedure.") } },


        { { "sub",  argtype_t::EAX,      argtype_t::imm32    }, { 0x2D, s("Add imm32 to EAX") } },
        { { "sub",  argtype_t::RAX,      argtype_t::imm32    }, { 0x2D, s("Add sign-extended imm32 to RAX") } },
        { { "sub",  argtype_t::regmem32, argtype_t::imm32    }, { 0x81, s("Add imm32 to reg/mem32"),               opcode_flags_t::reg_extended, 5 } },
        { { "sub",  argtype_t::regmem64, argtype_t::imm32    }, { 0x81, s("Add sign-extended imm32 to reg/mem64"), opcode_flags_t::reg_extended, 5 } },
        { { "sub",  argtype_t::regmem32, argtype_t::imm8     }, { 0x83, s("Add sign-extended imm8 to reg/mem32"),  opcode_flags_t::reg_extended, 5 } },
        { { "sub",  argtype_t::regmem64, argtype_t::imm8     }, { 0x83, s("Add sign-extended imm8 to reg/mem64"),  opcode_flags_t::reg_extended, 5 } },
        { { "sub",  argtype_t::regmem32, argtype_t::reg32    }, { 0x29, s("Add reg32 to reg/mem32") } },
        { { "sub",  argtype_t::regmem64, argtype_t::reg64    }, { 0x29, s("Add reg64 to reg/mem64") } },
        { { "sub",  argtype_t::reg32,    argtype_t::regmem32 }, { 0x2B, s("Add reg/mem32 to reg32") } },
        { { "sub",  argtype_t::reg64,    argtype_t::regmem64 }, { 0x2B, s("Add reg/mem64 to reg64") } },
    ...
    optional<instruction_t> parse_instruction(nextany_tokenizer::const_iterator_t& iter, const nextany_tokenizer::const_iterator_t& end) noexcept
    {
        ...
        do
        {
            ...
            do
            {
                ...
                if (test.arg2.operand_size() >= ret.signature.arg2.operand_size()
                    && test.arg1.operand_size() >= ret.signature.arg1.operand_size())
                {
                    break;
                }
            } while (true);
...
next power of two).
    ...
    {
        uint32_t cpuid[4]; 
        uint8_t __reserved2() { return (cpuid[0] >> 28) & (0x000F); }
        uint8_t ExtFamily() { return (cpuid[0] >> 20) & (0x00FF); }
        uint8_t ExtModel() { return (cpuid[0] >> 16) & (0x000F); }
        uint8_t __reserved1() { return (cpuid[0] >> 12) & (0x000F); }
        uint8_t BaseFamily() { return (cpuid[0] >> 8) & (0x000F); }
        uint8_t BaseModel() { return (cpuid[0] >> 4) & (0x000F); }
        uint8_t Stepping() { return (cpuid[0] >> 0) & (0x000F); }
        uint8_t LocalApicId() { return (cpuid[1] >> 24) & (0x00FF); }
        uint8_t LogicalProcessorCount() { return (cpuid[1] >> 16) & (0x00FF); }
        uint8_t ClFlush() { return (cpuid[1] >> 8) & (0x00FF); }
        uint8_t BrandId() { return (cpuid[1] >> 0) & (0x00FF); }
        bool RAZ() { return ((cpuid[2] >> 31) & 1) != 0 ? true : false; }
        bool RDRAND() { return ((cpuid[2] >> 30) & 1) != 0 ? true : false; }
        bool F16C() { return ((cpuid[2] >> 29) & 1) != 0 ? true : false; }
        bool AVX() { return ((cpuid[2] >> 28) & 1) != 0 ? true : false; }
        bool OSXSAVE() { return ((cpuid[2] >> 27) & 1) != 0 ? true : false; }
        bool XSAVE() { return ((cpuid[2] >> 26) & 1) != 0 ? true : false; }
        bool AES() { return ((cpuid[2] >> 25) & 1) != 0 ? true : false; }
        bool __reserved8() { return ((cpuid[2] >> 24) & 1) != 0 ? true : false; }
        bool POPCNT() { return ((cpuid[2] >> 23) & 1) != 0 ? true : false; }
        bool MOVEB() { return ((cpuid[2] >> 22) & 1) != 0 ? true : false; }
        bool __reserved7() { return ((cpuid[2] >> 21) & 1) != 0 ? true : false; }
        bool SSE4_2() { return ((cpuid[2] >> 20) & 1) != 0 ? true : false; }
        bool SSE4_1() { return ((cpuid[2] >> 19) & 1) != 0 ? true : false; }
        uint8_t __reserved6() { return (cpuid[2] >> 14) & (0x001F); }
        bool CMPXCHG16B() { return ((cpuid[2] >> 13) & 1) != 0 ? true : false; }
        bool FMA() { return ((cpuid[2] >> 12) & 1) != 0 ? true : false; }
        uint8_t __reserved5() { return (cpuid[2] >> 10) & (0x0003); }
        bool SSSE3() { return ((cpuid[2] >> 9) & 1) != 0 ? true : false; }
        uint8_t __reserved4() { return (cpuid[2] >> 4) & (0x001F); }
        bool MONITOR() { return ((cpuid[2] >> 3) & 1) != 0 ? true : false; }
        bool __reserved3() { return ((cpuid[2] >> 2) & 1) != 0 ? true : false; }
        bool PCLMULQDQ() { return ((cpuid[2] >> 1) & 1) != 0 ? true : false; }
        bool SSE3() { return ((cpuid[2] >> 0) & 1) != 0 ? true : false; }
        uint8_t __reserved13() { return (cpuid[3] >> 29) & (0x0007); }
        bool HTT() { return ((cpuid[3] >> 28) & 1) != 0 ? true : false; }
        uint8_t __reserved12() { return (cpuid[3] >> 25) & (0x0007); }
        bool SSE2() { return ((cpuid[3] >> 24) & 1) != 0 ? true : false; }
        bool SSE() { return ((cpuid[3] >> 23) & 1) != 0 ? true : false; }
        bool FXSR() { return ((cpuid[3] >> 22) & 1) != 0 ? true : false; }
        bool MMX() { return ((cpuid[3] >> 21) & 1) != 0 ? true : false; }
        bool __reserved11() { return ((cpuid[3] >> 20) & 1) != 0 ? true : false; }
        bool CLFSH() { return ((cpuid[3] >> 19) & 1) != 0 ? true : false; }
        bool __reserved10() { return ((cpuid[3] >> 18) & 1) != 0 ? true : false; }
        bool PSE36() { return ((cpuid[3] >> 17) & 1) != 0 ? true : false; }
        bool PAT() { return ((cpuid[3] >> 16) & 1) != 0 ? true : false; }
        bool CMOV() { return ((cpuid[3] >> 15) & 1) != 0 ? true : false; }
        bool MCA() { return ((cpuid[3] >> 14) & 1) != 0 ? true : false; }
        bool PGE() { return ((cpuid[3] >> 13) & 1) != 0 ? true : false; }
        bool MTRR() { return ((cpuid[3] >> 12) & 1) != 0 ? true : false; }
        bool SysEnterSysExit() { return ((cpuid[3] >> 11) & 1) != 0 ? true : false; }
        bool __reserved9() { return ((cpuid[3] >> 10) & 1) != 0 ? true : false; }
        bool APIC() { return ((cpuid[3] >> 9) & 1) != 0 ? true : false; }
        bool CMPXCHG8B() { return ((cpuid[3] >> 8) & 1) != 0 ? true : false; }
        bool MCE() { return ((cpuid[3] >> 7) & 1) != 0 ? true : false; }
        bool PAE() { return ((cpuid[3] >> 6) & 1) != 0 ? true : false; }
        bool MSR() { return ((cpuid[3] >> 5) & 1) != 0 ? true : false; }
        bool TSC() { return ((cpuid[3] >> 4) & 1) != 0 ? true : false; }
        bool PSE() { return ((cpuid[3] >> 3) & 1) != 0 ? true : false; }
        bool DE() { return ((cpuid[3] >> 2) & 1) != 0 ? true : false; }
        bool VME() { return ((cpuid[3] >> 1) & 1) != 0 ? true : false; }
        bool FPU() { return ((cpuid[3] >> 0) & 1) != 0 ? true : false; }
        /*
            push rbp
            push r9
            push r8
    ...
    error main() noexcept
    {
        ...
        int b = ((cd)mem)(info.cpuid, info.cpuid+1, info.cpuid+2, info.cpuid+3);
